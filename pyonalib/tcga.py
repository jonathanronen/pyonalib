...
CODE, DEFINITION, SHORT_CODE = list(
    zip(
        *[
            ["01", "Primary Solid Tumor", "TP"],
            ["02", "Recurrent Solid Tumor", "TR"],
...
PRIMARY_TUMOR_CODES = {
    code for code, defn in zip(CODE, DEFINITION) if "Primary" in defn
}


def select_primary_tumor_samples(df):
    primary_tumor_samples = [
        e for e in df.index if barcode2sampcode(e) in PRIMARY_TUMOR_CODES
    ]
    return df.loc[primary_tumor_samples]


RECURRENT_TUMOR_CODES = {
    code for code, defn in zip(CODE, DEFINITION) if "Recurrent" in defn
}


def select_recurrent_tumor_samples(df):
    recurrent_tumor_samples = [
        e for e in df.index if barcode2sampcode(e) in RECURRENT_TUMOR_CODES
    ]
    return df.loc[recurrent_tumor_samples]
